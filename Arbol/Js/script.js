...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con mucho cariño para ti";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2026-01-01T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2027-01-01T00:00:00');
  ...
  function update() {
    ...
    container.innerHTML =
      `Estamos juntos: <b>${days}</b> días<br>` +
...
if (musicaParam) {
  ...
  // Música por defecto
  audio.src = 'Music/TurningPage.mp3';
}
...
function createFallingHeart(xPosition) {
...
function startSideHearts() {
  setInterval(() => {
    const leftSide = document.querySelector(".side-images.left");
    const rightSide = document.querySelector(".side-images.right");

    if (leftSide) {
      const rect = leftSide.getBoundingClientRect();
      const x = rect.left + Math.random() * rect.width;
      createFallingHeart(x);
    }
...
window.addEventListener("DOMContentLoaded", startSideHearts);
